...
let color_graph vars live_sets formals static (type_map : type_map) program_points : coloring = 
  let is_empty ls = match ls with [] -> true | _ -> false in
  ...
  let writes = 
    List.fold_right
      (fun (k, v) acc -> VarMap.add k v acc)
      (ListPlus.zip
	 (VarSet.elements vars)
	 (List.map (fun id -> List.map (fun pp -> pp.index) (PPSet.elements (PPSet.filter (fun pp -> match pp.desc with Write id' -> id = id' | _ -> false) program_points))) (VarSet.elements vars)))
      VarMap.empty in
  ...
  let neighbours = 
    List.fold_right
      ...
      (ListPlus.zip
	 (VarSet.elements vars)
	 (List.map (fun id -> VarSet.remove id (List.fold_left VarSet.union VarSet.empty (List.filter (VarSet.mem id) live_sets))) (VarSet.elements vars)))
      VarMap.empty in
  ...
  let degrees = 
    List.fold_right
      ...
      (ListPlus.zip
	 (VarSet.elements vars)
	 (List.map (fun id -> VarSet.cardinal (VarMap.find id neighbours)) (VarSet.elements vars)))
      VarMap.empty in
  ...
  let color_compatability_map = 
    ...
    List.fold_right
      (fun ((a, b), v) acc -> VarVarMap.add (a, b) v (VarVarMap.add (b, a) v acc))
      (ListPlus.zip
	 pairs
	 (List.map (fun (a, b) -> color_compatible a b) pairs))
      VarVarMap.empty in
  ...
  let find_candidate_colors (id : Ast.identifier) (coloring : int VarMap.t) : int list =
    let rec iter (bindings : (VarMap.key * int) list) candidates rejected_colors =
	match bindings with
	  | [] ->
	    candidates
	  | (id', color) :: bindings' when color_compatible id id' && not (List.mem color rejected_colors) ->
	    iter bindings' (id' :: candidates) rejected_colors
	  | (_, color) :: bindings' ->
	    iter bindings' candidates (color :: rejected_colors) in
    iter (VarMap.bindings coloring) [] [] in
  ...
  let rec color_graph work_list next_available_color coloring =
	...
	coloring
      | next :: work_list' ->
	print_endline ("Considering " ^ next.Ast.identifier);
	let type_of_next = type_of next in
